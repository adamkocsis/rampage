...
Shorthand for limiting maximum and minimum values in a
`SpatRaster`-class object.
...
- y:
  ...
  Either a range (i.e. a `numeric` vector with two values), or
  `data.frame` with positioned color values (column `z` indicates
  values), or a calibrated ramp (e.g. produced with `expand`).
